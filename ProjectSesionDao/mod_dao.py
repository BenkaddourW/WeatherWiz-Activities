...
################################
@app.route('/v1/activities', methods=['DELETE'])
def delete_activity():

    data = request.get_json()
    if 'id' not in data:
        return jsonify({'error': 'ID non fourni'}), 400

    try:
        conn = creer_connexion()
        curseur = conn.cursor()

        # Vérification existence et suppression en une requête
        curseur.execute('DELETE FROM activite WHERE id = ? RETURNING id', [data['id']])
        deleted_row = curseur.fetchone()
        conn.commit()
